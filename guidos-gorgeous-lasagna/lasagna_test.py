...
# To that end, we are putting a try/catch around imports and throwing specific messages
# to help students
# decode that they need to create and title their constants and functions in a specific
# way.
try:
    from lasagna import (
        EXPECTED_BAKE_TIME,
...
# Here, we are separating the constant import errors from the function name import
# errors
    ...
    if "EXPECTED_BAKE_TIME" in item_name:
        ...
        raise ImportError(
            f"\n\nMISSING CONSTANT --> \nWe can not find or import the constant "
            f"{item_name} in your 'lasagna.py' file.\nDid you misname or forget to "
            f"define it?"
        ) from None
    else:
        ...
        # pylint: disable=raise-missing-from
        raise ImportError(
            f"\n\nMISSING FUNCTION --> In your 'lasagna.py' file, we can not find or "
            f"import the function named {item_name}. \nDid you misname or forget to "
            f"define it?"
        ) from None
        ...
        for variant, function in enumerate(functions, start=1):
            with self.subTest(f"variation #{variant}", function=function):
                ...
                failure_msg = (
                    ...
                    f"but the tests expected a docstring for the {function.__name__} "
                    f"function."
                )
